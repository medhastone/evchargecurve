...
import { 
  ...
  Car, Briefcase, Zap, AlertTriangle, Route, MapPin, PlusCircle
} from 'lucide-react';
...
export default function RangeLossTool() {
  ...
  const calcResult = useMemo(() => {
    ...
    // Additional stops for 300mi road trip (Assuming 10-80% charge usable per stop = 70% of max adjusted range)
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8" role="region" aria-label="EV Cold Weather and Highway Towing Range Calculator">
      
      {/* QUICK PRESETS */}
      <div className="mb-8 grid grid-cols-1 md:grid-cols-3 gap-4" role="group" aria-label="Simulation Presets">
        <button 
          type="button"
          onClick={() => applyPreset('winter')} 
          aria-label="Apply Sub-Zero Road Trip preset: 10°F, 70 mph, Resistive Max"
          className="bg-blue-500/10 border border-blue-500/30 hover:border-blue-500 p-4 rounded-2xl flex items-center gap-3 transition-colors text-left group"
        >
          <div className="bg-blue-500/20 p-2 rounded-lg text-blue-400 group-hover:scale-105 transition-transform">
            <ThermometerSnowflake className="w-5 h-5"/>
          </div>
          <div>
            <h3 className="font-bold text-blue-400 text-sm sm:text-base">Sub-Zero Road Trip</h3>
            <p className="text-xs text-blue-300/80">10°F (-12°C), {isMetric ? "112 km/h" : "70 mph"}, Resistive Max</p>
          </div>
        </button>

        <button 
          type="button"
          onClick={() => applyPreset('towing')} 
          aria-label="Apply Summer Boat Towing preset: 85°F, 65 mph, 4000 lbs"
          className="bg-orange-500/10 border border-orange-500/30 hover:border-orange-500 p-4 rounded-2xl flex items-center gap-3 transition-colors text-left group"
        >
          <div className="bg-orange-500/20 p-2 rounded-lg text-orange-400 group-hover:scale-105 transition-transform">
            <Car className="w-5 h-5"/>
          </div>
          <div>
            <h3 className="font-bold text-orange-400 text-sm sm:text-base">Summer Boat Towing</h3>
            <p className="text-xs text-orange-300/80">85°F (29°C), {isMetric ? "105 km/h" : "65 mph"}, 4000 lbs</p>
          </div>
        </button>
        ...
        <button 
          ...
          aria-label="Apply Normal Commute preset: 70°F, 55 mph, Heat Pump"
          className="bg-emerald-500/10 border border-emerald-500/30 hover:border-emerald-500 p-4 rounded-2xl flex items-center gap-3 transition-colors text-left group"
        >
          <div className="bg-emerald-500/20 p-2 rounded-lg text-emerald-400 group-hover:scale-105 transition-transform">
            <Route className="w-5 h-5"/>
          </div>
          <div>
            <h3 className="font-bold text-emerald-400 text-sm sm:text-base">Normal Commute</h3>
            <p className="text-xs text-emerald-300/80">70°F (21°C), {isMetric ? "88 km/h" : "55 mph"}, Heat Pump</p>
          </div>
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
        ...
        <div className="xl:col-span-6 space-y-6">
          ...
          <div className="bg-slate-800/50 border border-slate-700 p-6 rounded-2xl">
            <div className="flex items-center justify-between mb-2">
              <label htmlFor="range-loss-vehicle-select" className="text-xs text-slate-400 uppercase tracking-wider block font-semibold">
                Select EV Model
              </label>
              <button
                type="button"
              ...
              {customVehicles.length > 0 && (
                <optgroup label="⭐ My Custom Vehicles">
                  {customVehicles.map(v => (
                    <option key={v.id} value={v.id}>[Custom] {v.name} (EPA: {Math.round(isMetric ? (v.epaRangeMiles || 300) * 1.609 : (v.epaRangeMiles || 300))} {distanceLabel})</option>
                  ))}
                </optgroup>
              )}
          ...
          <div className="bg-slate-800/50 border border-slate-700 p-6 rounded-2xl space-y-8">
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                <label htmlFor="ambient-temp-slider" className="text-sm font-semibold text-slate-300 flex items-center gap-2">
              ...
              <input 
                ...
                aria-label="Ambient temperature slider"
                type="range" 
                min="-20" 
                max="105" 
                step="1"
            ...
            <div>
              <div className="flex justify-between items-center mb-3">
                <span id="cruising-speed-label" className="text-sm font-semibold text-slate-300 flex items-center gap-2">
                  <Wind className="w-4 h-4"/> Cruising Speed
                </span>
              </div>
              <div className="flex flex-wrap gap-2" role="radiogroup" aria-labelledby="cruising-speed-label">
                {[55, 65, 70, 75, 80].map(s => (
                  <button
                    key={s} 
                    type="button"
                    role="radio"
                    aria-checked={speedMph === s}
                    aria-label={`Select cruising speed ${isMetric ? Math.round(s * 1.609) : s} ${speedLabel}`}
                    onClick={() => setSpeedMph(s)}
                    className={cn(
                      "flex-1 py-2 text-sm font-bold rounded-xl border transition-all",
                      speedMph === s ? "bg-emerald-500/20 border-emerald-500/50 text-emerald-400" : "bg-slate-900 border-slate-700 text-slate-300 hover:border-slate-500"
                    )}
                  >
                    {isMetric ? Math.round(s * 1.609) : s} <span className="text-xs font-normal text-slate-400">{speedLabel}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div>
              <div className="flex justify-between items-center mb-3">
                <span id="climate-control-label" className="text-sm font-semibold text-slate-300">
                  Climate Control Mode
                </span>
              </div>
              <div className="grid grid-cols-2 gap-2" role="radiogroup" aria-labelledby="climate-control-label">
                {[
                  ...
                  <button
                    ...
                    type="button"
                    role="radio"
                    aria-checked={climateMode === c.id}
                    aria-label={`Select climate mode: ${c.label}`}
                    onClick={() => setClimateMode(c.id as ClimateMode)}
                    className={cn(
                      "py-2 text-xs font-semibold rounded-xl border transition-all",
                      climateMode === c.id ? "bg-cyan-500/20 border-cyan-500/50 text-cyan-400" : "bg-slate-900 border-slate-700 text-slate-300 hover:border-slate-500"
                    )}
                  >
                    {c.label}
                  </button>
            ...
            {/* Payload / Towing Slider */}
            <div>
              <div className="flex justify-between items-center mb-2">
                <label htmlFor="payload-towing-slider" className="text-sm font-semibold text-slate-300 flex items-center gap-2">
                  <Briefcase className="w-4 h-4"/> Towing / Payload Weight
                </label>
                <span className="text-sm font-bold text-emerald-400">{payloadLbs.toLocaleString()} lbs</span>
              </div>
              <input 
                id="payload-towing-slider"
                aria-label="Towing and payload weight slider"
                type="range" 
                min="0" 
                max="7500" 
                step="100"
                value={payloadLbs} 
                onChange={(e) => setPayloadLbs(Number(e.target.value))}
                className="w-full h-2 bg-slate-700 rounded-lg appearance-none cursor-pointer accent-emerald-500"
              />
            </div>
            ...
            <div>
              <div className="flex justify-between items-center mb-3">
                <span id="accessories-label" className="text-sm font-semibold text-slate-300">
                  Aero Accessories
                </span>
              </div>
              <div className="flex flex-col sm:flex-row gap-2" role="radiogroup" aria-labelledby="accessories-label">
                {[
                  { id: 'none', label: 'None' },
                  ...
                  <button
                    ...
                    aria-label={`Select aerodynamic accessory: ${a.label}`}
                    onClick={() => setAccessory(a.id as Accessory)}
                    className={cn(
                      "flex-1 py-2 text-xs font-semibold rounded-xl border transition-all",
                      accessory === a.id ? "bg-purple-500/20 border-purple-500/50 text-purple-400" : "bg-slate-900 border-slate-700 text-slate-300 hover:border-slate-500"
                    )}
                  >
                    {a.label}
                  </button>
        ...
        <div className="xl:col-span-6 space-y-6">
          ...
          <div className="bg-slate-800/50 border border-slate-700 p-6 rounded-2xl relative overflow-hidden">
            ...
            <h2 className="text-lg font-bold text-white mb-6">Real-World Range Estimation</h2>
            ...
            <div className="space-y-6">
              <div>
                ...
                <div 
                  role="progressbar" 
                  aria-label="Factory EPA Range baseline"
                  aria-valuenow={100}
                  aria-valuemin={0}
                  aria-valuemax={100}
                  className="w-full bg-slate-700 h-4 rounded-full overflow-hidden"
                >
              ...
              <div>
                ...
                <div 
                  role="progressbar"
                  aria-label="Calculated Real-World Highway Range"
                  aria-valuenow={Math.round(calcResult.adjustedRange)}
                  aria-valuemin={0}
                  aria-valuemax={Math.round(calcResult.baseEpaRange)}
                  className="w-full bg-slate-700 h-4 rounded-full overflow-hidden flex"
                >
                  <div 
                    className={cn(
                    ...
                    style={{ width: `${Math.max(0, Math.min(100, (calcResult.adjustedRange / calcResult.baseEpaRange) * 100))}%` }}
                  ></div>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-slate-800/50 border border-slate-700 p-4 rounded-2xl">
              ...
              <p className="text-xs text-slate-400 mt-2">{(isMetric ? calcResult.miPerKwh / 1.609 : calcResult.miPerKwh).toFixed(2)} {distanceLabel}/kWh</p>
            </div>
            ...
            <div className="bg-slate-800/50 border border-slate-700 p-4 rounded-2xl">
              ...
              <p className="text-xs text-slate-400 mt-2">Assuming 10-80% charges</p>
            </div>
          </div>
          ...
          <div className="bg-blue-500/10 border border-blue-500/20 p-5 rounded-2xl flex items-start gap-3">
            ...
            <div>
              <h3 className="text-sm font-bold text-blue-400 uppercase tracking-wider mb-2">Trip Planning Insight</h3>
              <p className="text-sm text-slate-200 leading-relaxed font-normal">
                A {isMetric ? "500-km" : "300-mile"} highway journey in these conditions will require charging stops to break up the drive. Relying on your vehicle&apos;s built-in navigation is recommended, as it automatically preconditions the battery pack before arrival at the DC fast charger, ensuring you get optimal charging speeds upon plug-in.
              </p>
            </div>
          </div>
